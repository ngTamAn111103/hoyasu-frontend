...
const HomeScreen = () => {
  ...
  // Vị trí giả định dành cho không có GPS 10.851548396166965, 106.75812846725613
  const initialRegion = {
  ...
  const handleCreateTripByVehicle = async (vehicleData) => {
    try {
      ...
      // --- 2. Lấy vị trí MỘT LẦN ---
      // (Bạn có thể thêm 1 Alert "Đang lấy GPS..." ở đây nếu muốn)
      let location;
      try {
        ...
        Alert.alert("Thất bại", "Không thể lấy được GPS! Vui lòng thử lại.");
        return;
      }

      // --- 3. Tạo dữ liệu chuyến đi ---
      const data = {
      ...
      if (result) {
        ...
        fetchActiveTrip(); // <-- Tốt! Dòng này sẽ kích hoạt useEffect ở Bước 2
      } else {
  ...
  useEffect(() => {
    // Nếu chưa đăng nhập -> Không cần Bật/Tắt GPS
    if (isAuthLoading || !isSignedIn) {
      return
    }
    ...
    if (activeTrip) {
      ...
      console.log("Bật theo dõi GPS cho chuyến đi...");
      setPath([]); // Xóa đường đi cũ
      startWatchingLocation();
    } else {
      // Không có chuyến đi -> Tắt GPS
      console.log("Tắt theo dõi GPS - Không có chuyến đi");
    ...
    <GestureHandlerRootView style={styles.container}>
      <View style={styles.container}>
        ...
        >
          ...
          - Nếu có GPS: Hiển thị trực tiếp currentLocation
          - Nếu không: Hiển thị vị trí được chỉ định initialRegion
          */}
          {
            region ? (
              // Vế 1: NẾU `currentLocation` tồn tại (true)
              <Marker coordinate={region} title="Vị trí của bạn" />
            ) : (
              // Vế 2: NẾU KHÔNG (false, currentLocation là null)
